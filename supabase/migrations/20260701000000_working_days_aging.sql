-- 1. Create helper function to calculate working days (Mon-Fri) difference
CREATE OR REPLACE FUNCTION public.get_working_days_diff(start_date DATE, end_date DATE)
RETURNS INTEGER
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
  ...
  WHILE curr <= end_date LOOP
    IF EXTRACT(ISODOW FROM curr) IN (1, 2, 3, 4, 5) THEN
...
-- 2. Update update_stagnant_leads function to use working days logic
...
DECLARE
  r RECORD;
BEGIN
  -- Iterate through active leads whose next follow-up date has passed (excluding weekends)
  FOR r IN 
    SELECT unique_id, lead_status, assigned_to, name, next_followup_date 
    FROM public.leads 
    WHERE next_followup_date IS NOT NULL
      AND public.get_working_days_diff(next_followup_date, CURRENT_DATE) >= 1
